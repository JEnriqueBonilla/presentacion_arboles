...
complicado que no conocemos y difícil de estimar. 
...
\end{aligned}
\end{equation*}$$ 
En la práctica, realmente nos interesa minimizar el error de predicción
...
es nuestro principal objetivo pues no revela no sólo como se 
...
o por métodos de reuso de muestras (validación cruzada, boostrap).
...
(se puede sobre-ajustar). Se recomienda, empíricamente, $25\%$ de
la muestra.
...
Si tenemos un menor tamaño de muestra, esta se separa sólo en
conjuntos ajenos de entrenamiento y validación. Con el conjunto
de entrenamiento estimamos $E_{rr}$ y comparamos-seleccionamos modelos.
...
el método aumenta el sesgo y, por lo ranto, aumenta el error.

- $Var_\gamma(x_0) = E_\gamma[(\hat{f}_\gamma(x_0)-E_\gamma[\hat{f}_\gamma(x_0)])^2]$
es la varianza del método. mide qué tanto varian las predicciones
...
Este comportamiento se encuentra, generalmente, independientemente 
...
#Datos: Munich Rent Standards (library(kknn))
